...
function DayBooksApp() {
  ...
  async function handleAddPartner({ name, equityPct }) {
    ...
    const total = partners.reduce((sum, partner) => sum + Number(partner.equity_pct || 0), 0);

    if (total + equity > 100) {
      showMessage(`Total would be ${total + equity}%. Max 100%.`, 'error');
      return;
    }
  ...
  async function handleEditPartner(id, { name, equityPct }) {
    ...
    const otherPartnersTotal = partners
      ...
      .reduce((sum, partner) => sum + Number(partner.equity_pct || 0), 0);

    if (otherPartnersTotal + equity > 100) {
      showMessage(`Total would be ${otherPartnersTotal + equity}%. Max 100%.`, 'error');
      return;
    }
  ...
  function isInFilter(dateValue, selectedFilter) {
    ...
    if (selectedFilter === 'today') return itemDate.toDateString() === now.toDateString();
    if (selectedFilter === 'week') return itemDate >= startOfWeek;
    if (selectedFilter === 'month') return itemDate >= startOfMonth;
  ...
  const periodTotals = useMemo(() => {
    function calc(period) {
      const income = entries
        .filter((item) => isInFilter(item.entry_date, period))
        .reduce((sum, item) => sum + Number(item.amount || 0), 0);

      const expense = expenses
        .filter((item) => isInFilter(item.expense_date, period))
        .reduce((sum, item) => sum + Number(item.amount || 0), 0);
