    ...
    async function loadEntries(shopId) {
        ...
        .eq('shop_id', shopId).order('entry_date', { ascending: false }).order('created_at', { ascending: false });
        if (error) { showMessage(error.message, 'error'); return; }
        setEntries(data || []);
    }
    ...
    async function loadExpenses(shopId) {
        ...
        .eq('shop_id', shopId).order('expense_date', { ascending: false }).order('created_at', { ascending: false });
        if (error) { showMessage(error.message, 'error'); return; }
        setExpenses(data || []);
    }
    ...
    // ── DELETE ACCOUNT ────────────────────────────────────────────────────────
    // Deletes all shop data, then signs out. The auth user deletion must be done
    // via Supabase dashboard or a server-side function — we clear all data here
    // and sign out, which effectively deactivates the account.
    ...
    async function handleDeleteAccount() {
        ...
        try {
        if (shop) {
            // Delete all shop data in order (foreign key safe)
        ...
        // Sign out — Supabase free tier doesn't allow client-side auth user deletion
        // The account is fully cleared of data. Auth record removal requires admin API.
        await signOut();
    ...
    async function handleAddPartner({ name, equityPct }) {
        ...
        if (currentTotal + equity > 100) { showMessage(`Total would be ${currentTotal + equity}%. Must not exceed 100%.`, 'error'); return; }
        setSubmitting(true);
    ...
    async function handleEditPartner(id, { name, equityPct }) {
        ...
        const equity = Number(equityPct);
        if (equity <= 0 || equity > 100) { showMessage('Equity must be between 1 and 100', 'error'); return; }
        const otherTotal = partners.filter(p => p.id !== id).reduce((sum, p) => sum + Number(p.equity_pct || 0), 0);
        if (otherTotal + equity > 100) { showMessage(`Total would be ${otherTotal + equity}%. Must not exceed 100%.`, 'error'); return; }
        setSubmitting(true);
    ...
    if (user && loadingShop) {
        ...
        <div style={st.loading}>
            ...
            <div style={st.loadingLogo}>D</div>
            <p style={st.loadingText}>Loading your shop...</p>
            </div>
        </div>
        );
    }
    ...
    if (user && !shop) {
        return <ShopSetup user={user} message={message} messageType={messageType} submitting={submitting} onCreateShop={handleCreateShop} onLogout={handleLogout} />;
    }
    ...
    loadingLogo: { width: '44px', height: '44px', background: '#C80815', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: '900', color: '#fff', margin: '0 auto 12px', fontFamily: "'Outfit', sans-serif" },
    loadingText: { color: '#555', fontSize: '14px', fontFamily: "'Outfit', sans-serif", margin: 0 },
    };