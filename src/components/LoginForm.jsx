...
export default function LoginForm({ mode, message, messageType, submitting, onSubmit, onToggleMode }) {
  ...
  // Forgot password state — handled locally, no prop needed
        ...
        <form onSubmit={handleSubmit} style={s.card}>
          ...
          {mode === 'login' && (
            <div style={s.forgotRow}>
              <button type="button" style={s.forgotBtn} onClick={() => { setForgotMode(true); setForgotEmail(email); }}>
                Forgot password?
              </button>
            </div>
          )}
...
const s = {
  ...
  input: {
    ...
    fontFamily: "'Outfit', sans-serif", width: '100%', boxSizing: 'border-box', transition: 'border-color 0.15s',
  },
  ...
  submitBtn: {
    ...
    fontFamily: "'Outfit', sans-serif", letterSpacing: '0.2px', marginTop: '4px', cursor: 'pointer',
  },
  toggleBtn: {
  ...
  sentWrap: { display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', textAlign: 'center', padding: '8px 0' },
  sentIcon: { fontSize: '40px', marginBottom: '4px' },
};