    ...
    return (
        ...
        <p style={s.hint}>Add each business owner with their ownership %. All partners should total 100%.</p>
        <div style={s.row2}>
            <Field label="Partner name" id="pf-name">
            <input id="pf-name" name="pf-name" type="text" placeholder="e.g. Mike" value={name} onChange={(e) => setName(e.target.value)} style={s.input} autoComplete="off" required />
            </Field>
            <Field label="Equity %" id="pf-equity">
            <input id="pf-equity" name="pf-equity" type="number" placeholder="50" value={equity} onChange={(e) => setEquity(e.target.value)} style={s.input} min="1" max="100" step="1" autoComplete="off" required />
            </Field>
        </div>
    ...
    return (
        ...
        <div style={s.inlineRow}>
            ...
            <input id="svf-name" name="svf-name" type="text" placeholder="e.g. Oil Change, Tire Rotation..." value={name} onChange={(e) => setName(e.target.value)} style={s.input} autoComplete="off" required />
            </div>
            <button type="submit" style={{ ...s.btn, padding: '10px 18px', flexShrink: 0, opacity: submitting ? 0.7 : 1, cursor: submitting ? 'not-allowed' : 'pointer' }} disabled={submitting}>
            {submitting ? '...' : 'Add'}
            </button>
        </div>
        </form>
    ...
    const CATEGORIES = ['Tire Shop','Auto Repair','Oil Change','Car Wash','Battery Shop','Brake & Exhaust','Other Auto'];
    ...
    return (
        ...
        <Field label="Business name" id="sf-name">
            <input id="sf-name" name="sf-name" type="text" placeholder="e.g. Mike's Tire & Auto" value={name} onChange={(e) => setName(e.target.value)} style={s.input} autoComplete="organization" required />
        </Field>

        <div style={s.row2}>
            <Field label="Business type" id="sf-cat">
            <select id="sf-cat" name="sf-cat" value={category} onChange={(e) => setCategory(e.target.value)} style={s.input}>
                <option value="">Select type...</option>
                {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            </Field>
            <Field label="Location (optional)" id="sf-loc">
            <input id="sf-loc" name="sf-loc" type="text" placeholder="City, State" value={location} onChange={(e) => setLocation(e.target.value)} style={s.input} autoComplete="street-address" />
            </Field>
        </div>
