...
export default function VerifyEmail({ email, onLogout, onConfirmed }) {
  ...
  const advancedRef = useRef(false); // prevent calling onConfirmed twice
  ...
  // Poll every 4s — auto-advance if user confirmed in another tab/device
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user?.email_confirmed_at && !advancedRef.current) {
  ...
  async function handleCheckNow() {
    ...
    setResendError('');
    try {
      // Try refreshing session first — this picks up confirmed status
      ...
      if (user?.email_confirmed_at && !advancedRef.current) {
        ...
        setResendError(
          "Your email isn't confirmed yet. Open the email we sent you and click the confirmation link, then come back here."
        );
      }
    } catch {
  ...
  async function handleResend() {
    ...
    try {
      ...
      if (error) { setResendError(error.message); } else {
...
const s = {
  ...
  spamNote: {
    fontSize: '13px', color: C.mid, background: C.bg,
  ...
  signoutBtn: {
    ...
    padding: '4px', textAlign: 'center', textDecoration: 'underline', textUnderlineOffset: '2px',
  },
};